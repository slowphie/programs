...
class Searcher(Displayable):
    ...
    def search(self):
        ...
        while not self.empty_frontier():
            ...
            if path.end() not in self.explored: #checking for cycles if the node has already been visited
                self.display(2, "Expanding:",path,"(cost:",path.cost,")")
                self.explored.add(path.end()) # adding the node to the explored nodes
                self.num_expanded += 1
...
if __name__ == "__main__":
    searchType = int(input("0 for Depth-First, 1 for A*"))
    if not searchType:
        print("Depth-First")
        mazeSearch = Searcher(searchProblem.maze)
        path1 = mazeSearch.search()
        print("Path found:", path1)

    else:
        print("A*")
        mazeSearch = AStarSearcher(searchProblem.maze)
        path2 = mazeSearch.search()
